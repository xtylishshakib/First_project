...
def DELETE_STUDENT(request,admin):
    ...
    student.delete()
    messages.success(request,'Record Are Successffuly Deleted')
    return redirect('view_student')
...
@login_required(login_url='/')
def VIEW_STAFF(requset):
    staff = Staff.objects.all()
    context = {
        'staff':staff,
    }
    return render(requset,'Hod/view_staff.html',context)
...
def UPDATE_STAFF(request):
    if request.method == "POST":
        ...
        staff.save()
        messages.success(request,'Staff is Successsfully Updated')
